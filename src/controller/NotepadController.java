...
public class NotepadController extends RootController {
	...
	public void openFile(DirItem dirItem) {
		// 检查是否已打开
		if (!((FileSystemController) RootController.controllers.get("controller.FileSystemController")).getFileSystem()
				.isOpen(dirItem)) {
			try {
				((FileSystemController) RootController.controllers.get("controller.FileSystemController"))
						.getFileSystem().open(dirItem, FileSystem.WRITE);
